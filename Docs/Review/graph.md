...
- **给定源节点**
  ...
  - 使用邻接矩阵存储, 有
    - 时间复杂度$O(V^2)$
      - 每次取某个节点的所有邻居时, 需要遍历那个节点对应的行, 则有$O(V)$的复杂度, 同时一共重复$O(V)$次(**每个节点最多入队一次**)
- 性质
...
- 要求
  ...
  - 可以存在权重为负的边
- 步骤
  - $l^k_{i,j}$为从$i$到$j$的中间节点仅包含$\{1,\cdots, k\}$的最短路径长度
  - $$l^{k}_{i,j} = \begin{cases}
    l^{k-1}_{i,j}&l^{k-1}_{i,j} \leq l_{i,k}^{k-1} + l_{k,j}^{k-1}\\
    l_{i,k}^{k-1} + l_{k,j}^{k-1}&l^{k-1}_{i,j} > l_{i,k}^{k-1} + l_{k,j}^{k-1}\\
  \end{cases}$$
- 细节
  - 按照点来扩张路径, 而非按照矩阵
